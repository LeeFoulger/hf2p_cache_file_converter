...
void run_converter(const char* map_path, const char* map_name)
{
	printf("%s\\%s.map\n", map_path, map_name);
	c_hf2p_cache_file_converter* converter = new c_hf2p_cache_file_converter(map_path, map_name);
...
int main(int argc, const char* argv[])
{
	if (argc <= 2)
	{
		...
		long filt = create_filter("", nullptr);

		char* filter = create_filters(filters);
		if (filter)
		{
			char* files_buffer = nullptr;
			long file_count = 64;
			if (get_file_name(filter, NULL, files_buffer, file_count, "", _get_file_name_type_open) && files_buffer)
			{
				char** strings = nullptr;
				long string_count = strings_from_buffer(files_buffer, strings, file_count);
				if (!strings)
				{
					delete[] filter;
					filter = nullptr;
				...
				if (string_count == 1)
				{
					map_path = strnew(files_buffer, PathRemoveFileSpecA);
					map_name = strnew(files_buffer + strlen(map_path) + 1, PathRemoveExtensionA);
					run_converter(map_path, map_name);
				}
				else for (int file_index = 1; file_index < string_count; file_index++)
				{
					map_path = strings[0];
					map_name = strnew(strings[file_index], PathRemoveExtensionA);
					run_converter(map_path, map_name);
				}
	...
	{
		run_converter(argv[1], argv[map_index]);
	}

	return 0;
}

c_cache_file_header::c_cache_file_header(unsigned char* data) :
...
c_hf2p_cache_file_converter::c_hf2p_cache_file_converter(const char* map_path, const char* map_name) :
	m_in_map_path(),
	m_in_map_data(),
	m_in_map_data_size(),
	m_out_map_path(),
...
{
	sprintf_s(m_in_map_path, "%s\\%s.map", map_path, map_name);
	sprintf_s(m_out_map_path, "%s\\%s_out.map", map_path, map_name);
	...
	for (long i = _cache_file_shared_tags; i < _cache_file_shared_tags + 1; i++)
	{
		...
		sprintf_s(m_shared_file_data_paths[i], 260, "%s\\%s.dat", map_path, k_cache_file_shared_type_names[i]);
	...
	m_out_map_data = new unsigned char[m_out_map_data_size] {};
...
{
	//char files_buffer[MAX_PATH]{};
	//if (get_file_name("*.map\0*.map\0\0", NULL, files_buffer, "", _get_file_name_type_save))
	//{
	//	return write_data_to_file(m_out_map_data, m_out_map_data_size, files_buffer);
	//}
...
{
	c_cache_file_tags_header& tags_header = *(new c_cache_file_tags_header(m_out_map_data + m_shared_file_data_offsets[_cache_file_shared_tags]));
